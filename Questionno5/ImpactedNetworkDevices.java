...
public class ImpactedNetworkDevices {
    // Finding nodes with only the target as parent
    public static List<Integer> findNodesWithOnlyTargetAsParent(int[][] edges, int target) {
        Map<Integer, List<Integer>> graph = new HashMap<>();
        Map<Integer, Integer> inDegree = new HashMap<>();
        // Building the graph and calculating in-degree of each node
        for (int[] edge : edges) {
            int from = edge[0];
            int to = edge[1];
        ...
        //  Depth-First Search (DFS) starting from the target node
